...
async function run() {
  const result = await pago.disputes.list({
...
import { disputesList } from "@pago-sh/sdk/funcs/disputesList.js";
...
async function run() {
  const res = await disputesList(pago, {
    organizationId: "1dbfc517-0bbf-4301-9ba8-555ca42b9737",
  });
  if (res.ok) {
...
async function run() {
  const result = await pago.disputes.get({
    id: "<value>",
  });
...
```typescript
import { PagoCore } from "@pago-sh/sdk/core.js";
import { disputesGet } from "@pago-sh/sdk/funcs/disputesGet.js";
...
async function run() {
  const res = await disputesGet(pago, {
    id: "<value>",
  });
  if (res.ok) {
    const { value: result } = res;
