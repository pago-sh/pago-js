...
async function run() {
  const result = await pago.customerPortal.customerMeters.list({
...
import { customerPortalCustomerMetersList } from "@pago-sh/sdk/funcs/customerPortalCustomerMetersList.js";
...
async function run() {
  const res = await customerPortalCustomerMetersList(pago, {
    customerSession: process.env["PAGO_CUSTOMER_SESSION"] ?? "",
  }, {});
  if (res.ok) {
...
async function run() {
  const result = await pago.customerPortal.customerMeters.get({
    customerSession: process.env["PAGO_CUSTOMER_SESSION"] ?? "",
  }, {
    id: "<value>",
  });
...
```typescript
import { PagoCore } from "@pago-sh/sdk/core.js";
import { customerPortalCustomerMetersGet } from "@pago-sh/sdk/funcs/customerPortalCustomerMetersGet.js";
...
async function run() {
  const res = await customerPortalCustomerMetersGet(pago, {
    customerSession: process.env["PAGO_CUSTOMER_SESSION"] ?? "",
  }, {
    id: "<value>",
  });
