...
async function run() {
  const result = await pago.customerPortal.downloadables.list({
...
import { customerPortalDownloadablesList } from "@pago-sh/sdk/funcs/customerPortalDownloadablesList.js";
...
async function run() {
  const res = await customerPortalDownloadablesList(pago, {
    customerSession: process.env["PAGO_CUSTOMER_SESSION"] ?? "",
  }, {});
  if (res.ok) {
