...
async function run() {
  const result = await pago.customerPortal.customerSession.introspect({
...
import { customerPortalCustomerSessionIntrospect } from "@pago-sh/sdk/funcs/customerPortalCustomerSessionIntrospect.js";
...
async function run() {
  const res = await customerPortalCustomerSessionIntrospect(pago, {
    customerSession: process.env["PAGO_CUSTOMER_SESSION"] ?? "",
  });
  if (res.ok) {
...
async function run() {
  const result = await pago.customerPortal.customerSession.getAuthenticatedUser({
    customerSession: process.env["PAGO_CUSTOMER_SESSION"] ?? "",
  });
...
```typescript
import { PagoCore } from "@pago-sh/sdk/core.js";
import { customerPortalCustomerSessionGetAuthenticatedUser } from "@pago-sh/sdk/funcs/customerPortalCustomerSessionGetAuthenticatedUser.js";
...
async function run() {
  const res = await customerPortalCustomerSessionGetAuthenticatedUser(pago, {
    customerSession: process.env["PAGO_CUSTOMER_SESSION"] ?? "",
  });
  if (res.ok) {
    const { value: result } = res;
