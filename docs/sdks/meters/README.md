...
async function run() {
  const result = await pago.meters.list({
...
import { metersList } from "@pago-sh/sdk/funcs/metersList.js";
...
async function run() {
  const res = await metersList(pago, {
    organizationId: "1dbfc517-0bbf-4301-9ba8-555ca42b9737",
  });
  if (res.ok) {
...
async function run() {
  const result = await pago.meters.create({
    name: "<value>",
    filter: {
      conjunction: "or",
      clauses: [
...
```typescript
import { PagoCore } from "@pago-sh/sdk/core.js";
import { metersCreate } from "@pago-sh/sdk/funcs/metersCreate.js";
...
async function run() {
  const res = await metersCreate(pago, {
    name: "<value>",
    filter: {
      conjunction: "or",
      clauses: [
...
async function run() {
  const result = await pago.meters.get({
    id: "<value>",
  });
...
```typescript
import { PagoCore } from "@pago-sh/sdk/core.js";
import { metersGet } from "@pago-sh/sdk/funcs/metersGet.js";
...
async function run() {
  const res = await metersGet(pago, {
    id: "<value>",
  });
  if (res.ok) {
    const { value: result } = res;
...
async function run() {
  const result = await pago.meters.update({
    id: "<value>",
    meterUpdate: {},
  });
...
```typescript
import { PagoCore } from "@pago-sh/sdk/core.js";
import { metersUpdate } from "@pago-sh/sdk/funcs/metersUpdate.js";
...
async function run() {
  const res = await metersUpdate(pago, {
    id: "<value>",
    meterUpdate: {},
  });
  if (res.ok) {
...
async function run() {
  const result = await pago.meters.quantities({
    id: "<value>",
...
```typescript
import { PagoCore } from "@pago-sh/sdk/core.js";
import { metersQuantities } from "@pago-sh/sdk/funcs/metersQuantities.js";
...
async function run() {
  const res = await metersQuantities(pago, {
    id: "<value>",
    startTimestamp: new Date("2025-11-25T04:37:16.823Z"),
    endTimestamp: new Date("2025-11-26T17:06:00.727Z"),
    interval: "day",
