...
async function run() {
  const result = await pago.refunds.list({
...
import { refundsList } from "@pago-sh/sdk/funcs/refundsList.js";
...
async function run() {
  const res = await refundsList(pago, {
    organizationId: "1dbfc517-0bbf-4301-9ba8-555ca42b9737",
  });
  if (res.ok) {
...
async function run() {
  const result = await pago.refunds.create({
    orderId: "<value>",
...
```typescript
import { PagoCore } from "@pago-sh/sdk/core.js";
import { refundsCreate } from "@pago-sh/sdk/funcs/refundsCreate.js";
...
async function run() {
  const res = await refundsCreate(pago, {
    orderId: "<value>",
    reason: "customer_request",
    amount: 90,
  });
