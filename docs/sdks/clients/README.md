...
async function run() {
  const result = await pago.oauth2.clients.create({
    redirectUris: [
...
import { oauth2ClientsCreate } from "@pago-sh/sdk/funcs/oauth2ClientsCreate.js";
...
async function run() {
  const res = await oauth2ClientsCreate(pago, {
    redirectUris: [
      "https://impolite-hippodrome.com/",
      "https://acidic-tomography.net/",
    ],
...
async function run() {
  const result = await pago.oauth2.clients.get({
    clientId: "<id>",
  });
...
```typescript
import { PagoCore } from "@pago-sh/sdk/core.js";
import { oauth2ClientsGet } from "@pago-sh/sdk/funcs/oauth2ClientsGet.js";
...
async function run() {
  const res = await oauth2ClientsGet(pago, {
    clientId: "<id>",
  });
  if (res.ok) {
    const { value: result } = res;
...
async function run() {
  const result = await pago.oauth2.clients.update({
    clientId: "<id>",
...
```typescript
import { PagoCore } from "@pago-sh/sdk/core.js";
import { oauth2ClientsUpdate } from "@pago-sh/sdk/funcs/oauth2ClientsUpdate.js";
...
async function run() {
  const res = await oauth2ClientsUpdate(pago, {
    clientId: "<id>",
    oAuth2ClientConfigurationUpdate: {
      redirectUris: [
        "https://classic-cantaloupe.org",
...
async function run() {
  const result = await pago.oauth2.clients.delete({
    clientId: "<id>",
  });
...
```typescript
import { PagoCore } from "@pago-sh/sdk/core.js";
import { oauth2ClientsDelete } from "@pago-sh/sdk/funcs/oauth2ClientsDelete.js";
...
async function run() {
  const res = await oauth2ClientsDelete(pago, {
    clientId: "<id>",
  });
  if (res.ok) {
    const { value: result } = res;
