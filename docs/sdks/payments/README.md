...
async function run() {
  const result = await pago.payments.list({
...
import { paymentsList } from "@pago-sh/sdk/funcs/paymentsList.js";
...
async function run() {
  const res = await paymentsList(pago, {
    organizationId: "1dbfc517-0bbf-4301-9ba8-555ca42b9737",
  });
  if (res.ok) {
...
async function run() {
  const result = await pago.payments.get({
    id: "<value>",
  });
...
```typescript
import { PagoCore } from "@pago-sh/sdk/core.js";
import { paymentsGet } from "@pago-sh/sdk/funcs/paymentsGet.js";
...
async function run() {
  const res = await paymentsGet(pago, {
    id: "<value>",
  });
  if (res.ok) {
    const { value: result } = res;
