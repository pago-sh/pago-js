...
async function run() {
  const result = await pago.events.list({
...
import { eventsList } from "@pago-sh/sdk/funcs/eventsList.js";
...
async function run() {
  const res = await eventsList(pago, {
    organizationId: "1dbfc517-0bbf-4301-9ba8-555ca42b9737",
  });
  if (res.ok) {
...
async function run() {
  const result = await pago.events.listNames({
    organizationId: "1dbfc517-0bbf-4301-9ba8-555ca42b9737",
  });
...
```typescript
import { PagoCore } from "@pago-sh/sdk/core.js";
import { eventsListNames } from "@pago-sh/sdk/funcs/eventsListNames.js";
...
async function run() {
  const res = await eventsListNames(pago, {
    organizationId: "1dbfc517-0bbf-4301-9ba8-555ca42b9737",
  });
  if (res.ok) {
    const { value: result } = res;
...
async function run() {
  const result = await pago.events.get({
    id: "<value>",
  });
...
```typescript
import { PagoCore } from "@pago-sh/sdk/core.js";
import { eventsGet } from "@pago-sh/sdk/funcs/eventsGet.js";
...
async function run() {
  const res = await eventsGet(pago, {
    id: "<value>",
  });
  if (res.ok) {
    const { value: result } = res;
...
async function run() {
  const result = await pago.events.ingest({
    events: [],
  });
...
```typescript
import { PagoCore } from "@pago-sh/sdk/core.js";
import { eventsIngest } from "@pago-sh/sdk/funcs/eventsIngest.js";
...
async function run() {
  const res = await eventsIngest(pago, {
    events: [],
  });
  if (res.ok) {
    const { value: result } = res;
